...
async def analyze_check_debug(
    ...
    try:
        ...
        # Validate file exists and belongs to debug user
        file_result = await db.execute(
            select(FileRecord)
            .where(FileRecord.id == request.file_id)
        ...
        if not file_record:
            raise HTTPException(
                status_code=404,
                detail="File not found or doesn't belong to debug user"
            )
        ...
        # Create a simplified analysis record without actual processing
        analysis_record = AnalysisResult(
            id=str(uuid.uuid4()),
            file_id=request.file_id,
            analysis_timestamp=datetime.now(timezone.utc),
            
            # Mock forensics results
            forensics_score=45.0,
            edge_inconsistencies={"anomalies": ["edge_blur_detected"], "score": 0.3},
            compression_artifacts={"score": 0.2, "artifacts": ["jpeg_compression"]},
            font_analysis={"score": 0.4, "inconsistencies": ["font_variation"]},
            ...
            rule_violations={
                "violations": ["Amount exceeds threshold", "Signature verification failed"],
                "passed_rules": ["Date format valid", "Payee name valid"],
                "rule_scores": {"amount_check": 0.8, "signature_check": 0.7, "date_check": 0.3}
            },
            confidence_factors={"overall": 0.75, "forensics": 0.6, "ocr": 0.85, "rules": 0.8}
        )
        ...
        logger.error(f"Debug analysis failed for file {request.file_id}: {str(e)}")
        raise HTTPException(
            status_code=500,
