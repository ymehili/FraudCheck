...
@router.get("/stats-debug", response_model=DashboardStats)
async def get_dashboard_stats_debug(
    db: AsyncSession = Depends(get_db)
):
    """
    Debug endpoint to get dashboard statistics without authentication.
    """
    try:
        # Use debug user ID
        ...
        # Get stats for debug user
        return await get_dashboard_stats(user, db)
        
    except Exception as e:
        logger.error(f"Failed to get debug dashboard stats: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve debug dashboard statistics: {str(e)}"
        )


@router.get("/history-debug", response_model=AnalysisHistoryResponse)
async def get_analysis_history_debug(
    filters: DashboardFilter = Depends(),
    pagination: PaginationParams = Depends(),
    db: AsyncSession = Depends(get_db)
):
    """
    Debug endpoint to get analysis history without authentication.
    """
    try:
        # Use debug user ID
        debug_user_id = "debug_user_123"
        ...
        # Get history for debug user
        return await get_analysis_history(filters, pagination, user, db)
        
    except Exception as e:
        logger.error(f"Failed to get debug analysis history: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve debug analysis history: {str(e)}"
        )